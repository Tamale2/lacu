...
(function ($) {
  $(document).ready(function () {
    var $body = $("body");
    var $block_bios = $(".block--bios");
    var $block_profiles = $(".block--profiles");
    var $block_board = $(".block--board");
    var $block_profiles_details;

    $block_profiles.parent().after($block_board.parent());

    $block_profiles.wrapInner("<div class='bios--slides'></div>");

    wrapBioLinks();

    $(".bios--slides").on('init', function (event, slick) {
      setTimeout(function () {
        setActiveBio(0);
        // updateCurrentBio(event, slick, 0);
      }, 300);
    });

    $(".bios--slides").on('afterChange', function (event, slick, currentSlide) {
      updateBioDetails();
    });

    $(".bios--slides").on('swipe', function (event, slick, currentSlide) {
      updateBioDetails();
    });

    $(".bios--slides").on('setPosition', function (event, slick, currentSlide) {
    ...
    $(".bios--slides").slick({
      ...
      swipeToSlide: true,
      draggable: true,
      prevArrow: '<button class="slick-prev"><svg xmlns="http://www.w3.org/2000/svg" width="28" height="50" viewBox="0 0 28 50"><polygon fill="#084C85" fill-rule="evenodd" points="26.351 0 1.768 23.447 1.679 23.361 .036 24.928 .126 25.014 0 25.133 1.647 26.705 1.773 26.585 26.319 49.996 27.962 48.431 3.415 25.017 27.998 1.57"/></button>',
      nextArrow: '<button class="slick-next"><svg xmlns="http://www.w3.org/2000/svg" width="28" height="50" viewBox="0 0 28 50"><polygon fill="#084C85" fill-rule="evenodd" points="1.647 0 26.23 23.447 26.319 23.361 27.962 24.928 27.872 25.014 27.998 25.133 26.351 26.705 26.225 26.585 1.679 49.996 .036 48.431 24.583 25.017 0 1.57"/></svg></button></button>',
      responsive: [{
        breakpoint: 1199,
        settings: {
          slidesToShow: 5,
    ...
    function wrapBioLinks() {
      $(".block--profiles .bio__profile").wrapInner("<a href='#' class='bio__link'></a>");
      $block_profiles_details = $("<div class='block__profiles__details'></div>");
      $block_profiles_details.appendTo($block_profiles);
    }

    function handleBioTap(e) {
      handleBioClick(e);
      e.preventDefault();
    }
    ...
    function handleBioClick(e) {
      // console.log("-----",
      //   $(e.currentTarget).parent().parent().parent().attr("data-slick-index"),
      //   $(".slick-current.slick-center .bio__link").parent().parent().parent().attr("data-slick-index"),
      //   "-----");
      e.preventDefault();
      var slideIndex = $(e.currentTarget).parent().parent().parent().attr("data-slick-index");
      setActiveBio(slideIndex);
      // if(!$(e.currentTarget).hasClass("is-active")) {
      //   $('.block--profiles .bio__link').removeClass("is-active");
      //   $(".slick-current.slick-center .bio__link").addClass("is-active");
      //   var bio = $(e.currentTarget).parent().parent();
      //   var details = bio.find(".bio__details").clone();
      //   var slideIndex = $(e.currentTarget).parent().parent().parent().attr("data-slick-index");
      //   console.log(slideIndex);
      //   $(".bios--slides").slick('slickGoTo', parseInt(slideIndex) );
      //   setBioDetails(details);
      // }
    }

    function setActiveBio(slideIndex) {
      $(".bios--slides").slick('slickGoTo', parseInt(slideIndex));
      updateBioDetails();
    }

    function updateBioDetails() {
      // var $currentSlide = $(".slick-current.slick-center");
      var $currentSlide = $(".block--profiles .slick-active:eq(0)");
      $('.block--profiles .bio__link').removeClass("is-active");
      // $(".slick-current.slick-center .bio__link").addClass("is-active");
      $(".block--profiles .slick-active:eq(0) .bio__link").addClass("is-active");
      var details = $currentSlide.find(".bio__details").clone();
      setBioDetails(details);
    }

    function setBioDetails(details) {
      $block_profiles_details.html(details);
    }

    function updateCurrentBio(event, slick, currentSlide) {
      // console.log("CURRENT:", $(".bios--slides").find(".slick-current.slick-center .bio__link"));
      // $(".bios--slides").find(".slick-current.slick-center .bio__link").click();
      // $(".bios--slides").slick('slickGoTo', currentSlide );
      // $('.block--profiles .bio__link').removeClass("is-active");
      // $(".slick-current.slick-center .bio__link").addClass("is-active");
      // var bio = $(e.currentTarget).parent().parent();
      // var details = bio.find(".bio__details").clone();
    ...
    $body.delegate('.block--profiles .bio__link', {
      touchend: handleBioTap,
      click: handleBioClick
    });

    // $('.block--profiles .bio__link').first().click();


    //  TABS


    if ($('.block--bios--tabbed').length > 0) {
      createTabbedNavigation();
    }

    function createTabbedNavigation() {
      $block_profiles.parent().wrapAll("<div class='tabbed--area' id='tab--profiles'></div>");
      $block_board.parent().wrapAll("<div class='tabbed--area' id='tab--board' aria-hidden='true'></div>");

      var tab_list = [];
      tab_list.push({ title: "Leadership", area: "tab--profiles" });
      tab_list.push({ title: "Board", area: "tab--board" });

      var tabs = makeTabs(tab_list);

      $("#tab--profiles").parent().before(tabs);
    ...
    function makeTabs(list) {
      var tabs = $("<ul class='nav tabs'></ul>");

      for (var i = 0; i < list.length; i++) {
        var l = list[i];
        var tab = $("<li class='tabs__item'></li>");
        var link = $("<a href='#' area-controls='" + l.area + "'>" + l.title + "</a></li>");
        link.appendTo(tab);
        link.on("click", setActiveTab);
        tab.appendTo(tabs);
      }
