...
(function ($) {
  var hp_modal = Cookies.get('hp_modal');
  if (hp_modal === undefined) {
    hp_modal = 1;
  } else {
    hp_modal++;
  }
  Cookies.set('hp_modal', hp_modal, { expires: 30 });

  $(document).ready(function () {
    if (hp_modal <= max_modal_occurrence || max_modal_occurrence == 0) {
      $("#modal_content").dialog({
        width: 'auto',
        height: 'auto',
        autoOpen: true,
        modal: true,
        position: { my: "center top", at: "center top+" + modal_y_pos, of: window }
      });
    }
  });
})(jQuery);
